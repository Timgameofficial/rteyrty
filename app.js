...
(function () {
  ...
  /* ---------------------------------------------------------
     0a2. HAPTIC + SPRING — спільні утиліти для дискретних дій
     --------------------------------------------------------- */
  function haptic(ms) {
    if (reduceMotion) return;
    if (window.navigator && typeof window.navigator.vibrate === "function") {
      try { window.navigator.vibrate(ms || 10); } catch (e) {}
    }
  }

  /* Ручна пружинна фізика (без бібліотек): недодемпфована система
     дає легкий, контрольований overshoot і природне заспокоєння —
     саме те відчуття "drag -> release -> overshoot -> settle". */
  function springScrollTo(el, target, velocity, onDone) {
    if (reduceMotion) {
    ...
    function frame() {
      ...
      pos += vel;
      el.scrollLeft = pos;
      if (Math.abs(target - pos) > 0.4 || Math.abs(vel) > 0.4) {
  ...
  /* ---------------------------------------------------------
     ...
     Фокус-зона дає лёгкий "оптичний" glass-акцент і короткий
     haptic-тик щойно новий пункт фіксується в центрі.
     --------------------------------------------------------- */
  function initModeNav(track, opts) {
    ...
    items.forEach(function (item) {
      ...
      try {
        ...
        if (u.origin === location.origin && p === here) {
    ...
    if (!isTouch) {
      ...
      function endDrag() {
        ...
        /* легкий fling-снеп до найближчого пункту з урахуванням швидкості відпускання */
        var rect = track.getBoundingClientRect();
        var refX = opts.center ? rect.left + rect.width / 2 : rect.left + 30;
        var projected = track.scrollLeft + velocity * 5.5;
        var maxScroll = track.scrollWidth - track.clientWidth;
        projected = Math.max(0, Math.min(maxScroll, projected));
        var best = null, bestDist = Infinity;
        items.forEach(function (item) {
          var target = track.scrollLeft + (item.getBoundingClientRect().left + item.offsetWidth / 2) - refX;
          var dist = Math.abs(target - projected);
          if (dist < bestDist) { bestDist = dist; best = target; }
        });
        if (best !== null) springScrollTo(track, best, -velocity * 0.6, update);
      }
  ...
  initModeNav(document.querySelector(".main-nav ul"), { center: true });
  ...
  /* ---------------------------------------------------------
     5b. LIGHTBOX — повноекранна галерея у стилі Photos: тап на
     мініатюру "розкриває" її на весь екран (FLIP-переліт із точки
     кліку), свайп вліво/вправо гортає, свайп вниз закриває.
     --------------------------------------------------------- */
  (function initLightbox() {
    var items = Array.prototype.slice.call(document.querySelectorAll(".gallery-item img"));
    if (!items.length) return;

    var lightbox, stage, imgEl, captionEl;
    var index = 0;
    var built = false;
    ...
    function build() {
      ...
      lightbox.innerHTML =
        '<button type="button" class="lightbox-close" aria-label="Закрити"><svg viewBox="0 0 20 20"><path d="M4 4l12 12M16 4L4 16" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round"/></svg></button>' +
        '<button type="button" class="lightbox-nav lightbox-prev" aria-label="Попереднє фото"><svg viewBox="0 0 12 20"><path d="M10 2L2 10l8 8" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round"/></svg></button>' +
        '<button type="button" class="lightbox-nav lightbox-next" aria-label="Наступне фото"><svg viewBox="0 0 12 20"><path d="M2 2l8 8-8 8" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round"/></svg></button>' +
        '<div class="lightbox-stage"><img class="lightbox-img" alt=""></div>' +
        '<div class="lightbox-caption" data-lb-caption></div>';
      document.body.appendChild(lightbox);
      stage = lightbox.querySelector(".lightbox-stage");
      imgEl = lightbox.querySelector(".lightbox-img");
      captionEl = lightbox.querySelector("[data-lb-caption]");

      lightbox.querySelector(".lightbox-close").addEventListener("click", close);
      lightbox.querySelector(".lightbox-prev").addEventListener("click", function () { go(-1); });
      lightbox.querySelector(".lightbox-next").addEventListener("click", function () { go(1); });
      ...
      /* свайп: вліво/вправо — гортання, вниз — закриття (як Photos) */
      ...
      stage.addEventListener("pointermove", function (e) {
        ...
        if (Math.abs(dy) > Math.abs(dx)) {
          imgEl.style.transform = "translateY(" + dy + "px) scale(" + Math.max(.72, 1 - Math.abs(dy) / 800) + ")";
          lightbox.style.background = "color-mix(in srgb, var(--bone) " + Math.max(30, 92 - Math.abs(dy) / 3) + "%, transparent)";
        } else {
      ...
      stage.addEventListener("pointerup", function () {
        ...
        if (Math.abs(dy) > Math.abs(dx) && dy > 110) {
          close();
        } else if (dx > 70) {
          go(-1);
        } else if (dx < -70) {
    ...
    function render() {
      var src = items[index].currentSrc || items[index].src;
      imgEl.src = src;
      imgEl.alt = items[index].alt || "";
      var fig = items[index].closest("figure");
  ...
  document.querySelectorAll("[data-ba]").forEach(function (slider) {
    ...
    var lastZone = null;
    function zoneOf(v) {
      if (v <= 4) return "before";
      if (v >= 96) return "after";
      if (v >= 46 && v <= 54) return "mid";
      return null;
    }

    function setPos(val, opts) {
      opts = opts || {};
      var v = Math.max(0, Math.min(100, val));
      slider.style.setProperty("--ba-pos", v + "%");
      range.value = v;
      /* короткий тактильний тик лише при вході в ключову зону
         (до / середина / після), не на кожному пікселі руху */
      if (!opts.silent) {
        var zone = zoneOf(v);
        if (zone && zone !== lastZone) haptic(zone === "mid" ? 6 : 10);
        lastZone = zone;
      }
    }
    ...
    ["pointerup", "pointercancel", "pointerleave"].forEach(function (ev) {
      slider.addEventListener(ev, function () {
        if (!active) return;
        active = false;
        /* легкий магнітний snap до країв, якщо відпустили зовсім близько */
        var v = parseFloat(range.value);
        if (v > 0 && v < 6) setPos(0);
        else if (v < 100 && v > 94) setPos(100);
      });
    });
  ...
  /* ---------------------------------------------------------
     9. АКОРДЕОНИ: справжня height 0 -> auto анімація (grid-trick)
     поверх нативного <details>, з прогресивним посиленням —
     без JS <details> продовжує працювати миттєво, нічого не рветься.
     У групі price-accordion відкритий лише один пункт.
     --------------------------------------------------------- */
  function closeAccAnimated(acc) {
    if (!acc.classList.contains("is-open-visual")) {
      acc.removeAttribute("open");
      return;
    }
    acc.classList.remove("is-open-visual");
    window.setTimeout(function () {
      acc.removeAttribute("open");
    }, 520);
  }
  function openAccAnimated(acc) {
    acc.setAttribute("open", "");
    void acc.offsetWidth; /* форсуємо reflow, щоб transition стартував з 0fr */
    requestAnimationFrame(function () {
      acc.classList.add("is-open-visual");
    });
  }
  document.querySelectorAll(".acc").forEach(function (acc) {
    acc.classList.add("js-acc");
    if (acc.hasAttribute("open")) acc.classList.add("is-open-visual");
    var summary = acc.querySelector("summary");
    if (!summary) return;
    summary.addEventListener("click", function (e) {
      e.preventDefault();
      var isOpen = acc.classList.contains("is-open-visual");
      var group = acc.closest(".price-accordion");
      if (isOpen) {
        closeAccAnimated(acc);
      } else {
        if (group) {
          Array.prototype.slice.call(group.querySelectorAll(".acc")).forEach(function (other) {
            if (other !== acc && other.classList.contains("is-open-visual")) closeAccAnimated(other);
          });
        }
        openAccAnimated(acc);
      }
